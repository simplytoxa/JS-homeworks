(() => {
  ...
  new Promise(function(resolve) {
    ...
    results.addEventListener('click', function(e) {
      // =======================================================================================
      // My code! Change time by click on progressbar
      if (e.target.getAttribute('data-role') === 'progress-line' || e.target.getAttribute('data-role') === 'progressbar') {
        let progBar        = e.target.getAttribute('data-role') === 'progress-line' ? e.target : e.target.closest('.progress'),
            progressWidth  = progBar.clientWidth,
            offsetProgress = e.offsetX,
            progress       = progressWidth - (progressWidth - offsetProgress),
            newTime        = globalPlayer.duration / progressWidth * progress,
            progressBar    = e.target.querySelector('[data-role=progressbar]') || e.target;


        if (globalPlayer.currentTime) {
          if (progBar === playingItem.querySelector('.progress')) {
            progressBar.style.width = progress + 'px';
            globalPlayer.currentTime = newTime;
          }
        }else {
          alert('Choose the song!');
        }
      }
      // ======================================================================================
      // End of my code
